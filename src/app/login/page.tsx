...
import { Zap, Eye, EyeOff, Mail, Lock, Loader2, Shield, Trophy, TrendingUp } from "lucide-react";
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 gradient-hero-premium relative items-center justify-center p-12">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute top-1/4 left-1/4 w-72 h-72 bg-[#00d46e] rounded-full blur-[180px] opacity-[0.06]" />
          <div className="absolute bottom-1/4 right-1/4 w-56 h-56 bg-[#3b82f6] rounded-full blur-[150px] opacity-[0.05]" />
        </div>
        <div className="relative max-w-md">
          ...
          <h2 className="text-3xl font-extrabold text-white leading-tight mb-4">
            Your Next Big Win<br />Starts Here.
          </h2>
          <p className="text-base text-[#8b95b8] mb-8">
            Live odds from 16+ leagues. Instant deposits via Paystack & Mobile Money. Fast payouts guaranteed.
          </p>
          <div className="space-y-4">
            {[
              { icon: <TrendingUp className="w-4 h-4" />, text: "Real-time odds from The Odds API", color: "text-[#00d46e]" },
              { icon: <Shield className="w-4 h-4" />, text: "Licensed, secure, and regulated", color: "text-[#3b82f6]" },
              { icon: <Trophy className="w-4 h-4" />, text: "GHS 284K+ won by players today", color: "text-[#ffc107]" },
            ].map((item) => (
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12 bg-[#0f1118]">
        <div className="w-full max-w-md fade-in-up">
          ...
          <div className="mb-6 lg:mb-8">
            ...
            <p className="text-sm text-[#5a6485]">Sign in to your account to continue</p>
          </div>
          ...
          <div className="bg-[#161925] border border-[#2a3050] rounded-2xl p-6 sm:p-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Email */}
              <div>
                <label className="text-xs font-medium text-[#8b95b8] mb-2 block">Email Address</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-medium text-[#8b95b8]">Password</label>
                  <Link href="/forgot-password" className="text-[11px] text-[#3b82f6] hover:underline">Forgot password?</Link>
                </div>
                <div className="relative">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${
                    rememberMe ? "bg-[#00d46e] border-[#00d46e]" : "border-[#2a3050] bg-[#0f1118]"
                  }`}
                >
                  {rememberMe && (
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full gradient-green text-white font-bold text-sm py-3.5 rounded-xl hover:opacity-90 transition-all disabled:opacity-60 flex items-center justify-center gap-2 glow-green"
              >
          ...
          <div className="flex items-center justify-center gap-4 mt-6">
            {["18+", "SSL Secured", "Licensed"].map((badge) => (
              <span key={badge} className="text-[10px] text-[#5a6485] bg-[#1c2033] px-2.5 py-1 rounded-lg border border-[#2a3050]">
                {badge}
              </span>
            ))}
          </div>
